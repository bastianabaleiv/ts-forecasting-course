...
Transformaciones tales como logarítmos (o Box-Cox) pueden ayudar a estabilizar la varianza de una serie de tiempo. En este caso, la diferenciación logra estabilizar la media de una serie de tiempo removiendo los cambios en el nivel de una serie, y por ende, eliminado (o reduciendo) su tendencia y estacionalidad. Tal como vimos en clases previas, el gráfico de la ACF es útil para identificar series de tiempo no estacionarias^[La ACF de una serie de tiempo estacionaria decae a cero relativamente rápido, mientras que para series no estacionarias lo hace lentamente].  
...
::: {.panel}
[AirPassengers 1st diff]{.panel-name}
```{r}
AirPassengers %>% 
  timetk::tk_tbl() %>% 
...
::: {.panel}
[AirPassengers 1st + seasonal diffs]{.panel-name}
```{r}
AirPassengers %>%
  timetk::tk_tbl() %>%
  mutate(
...
::: {.panel}
[ACF/PACF AirPassengers]{.panel-name}
```{r}
AirPassengers %>% 
  timetk::tk_tbl() %>% 
  timetk::plot_acf_diagnostics(
...
::: {.panel}
[ACF/PACF AirPassengers 1st diff]{.panel-name}
```{r}
AirPassengers %>% 
  timetk::tk_tbl() %>% 
  mutate(diff_value = timetk::diff_vec(value, lag = 1)) %>%
...
::: {.panel}
[ACF/PACF AirPassengers 1st + seasonal diffs]{.panel-name}
```{r}
AirPassengers %>%
  timetk::tk_tbl() %>%
  mutate(
...
La implementación de `auto.arima()`^[Paper: [Automatic time series forecasting: the forecast package for R](http://www.jstatsoft.org/v27/i03)] es una algoritmo que funciona del siguiente modo:

+ Selecciona la cantidad de diferencias `d` mediante tests de raíz unitaria.
+ Selecciona `p` y `q` minimizando $AIC_{c}$.
+ Estima los parámetros del modelo mediante la estimación de su máxima verosimilitud.
+ Busca a través del espacio de modelos por pasos (*stepwise*).
...
el modelo ajustado posee estacionalidad en una media móvil, `sma1`. El modelo ajustado
corresponde a un modelo `ARIMA(1,0,2)(0,1,1)[12]`, es decir, un modelo ARIMA con
componente estacional dada por la forma `ARIMA(p,d,q)(P,D,Q)[m]`, donde:

+ `d` es la cantidad de diferencias con respecto al rezago 1 
+ `p` es la cantidad de rezagos AR ordinarios: $y_{t-1},y_{t-2},\dots,y_{t-p}$ 
+ `q` es la cantidad de rezagos MA ordinarios: $\varepsilon_{t-1},\varepsilon_{t-2},\dots,\varepsilon_{t-q}$ 
+ `D` es la cantidad de diferencias estacionales 
+ `P` es la cantidad de rezagos AR estacionales: $y_{t-m},y_{t-2m},\dots,y_{t-Pm}$ 
+ `Q` es la cantidad de rezagos MA estacionales: $\varepsilon_{t-m},\varepsilon_{t-2m},\dots,\varepsilon_{t-Qm}$ 
+ `m` es la cantidad de observaciones por ciclo^[En este caso, en un año.]


# Modelos de Regresión Dinámica 📈

Hasta ahora hemos revisado modelos de pronóstico que utilizan sólo la historia de la serie, 
sin considerar información adicional que podría estar disponible. Pero a menudo puede existir
información adicional^[Feriados, información de la competencia, etc.] que nos permita mejorar las predicciones.

Los *Modelos de Regresión Dinámica* (*Dynamic Regression Models*) corresponden a un modelo de regresión lineal
con errores ARIMA:

$$y_t = \beta_0 + \beta_{1}x_{1,t} + \dots + \beta_{r}x_{r,t} + \varepsilon_t$$

donde $y_t$ es modelado como función de $r$ variables independientes y el término del error es un
proceso ARIMA (en lugar del ruido blanco usual).

# Modelos para estacionalidades complejas 📈

## TBATS

Una aproximación alternativa a los modelos de regresión dinámica es el modelo `TBATS`^[**T**: estacionalidad trigonométrica, **B**: transformación Box-Cox, **A**: errores ARIMA, **T**: tendencia, **S**: componentes estacionales]^[Paper: [Forecasting Time Series With Complex Seasonal Patterns Using Exponential Smoothing](https://doi.org/10.1198/jasa.2011.tm09771)] que considera^[Puedes encontrar una buena comparación del modelo TBATS con regresión Dinámica acá: [Forecasting weekly data](https://robjhyndman.com/hyndsight/forecasting-weekly-data/)]:

+ Términos trigonométricos para la estacionalidad^[Admite múltiples estacionalidades]
+ Transformaciones de Box-Cox para heterogeneidad.
+ Errores ARMA para dinámica de corto plazo.
+ Tendencia^[Usualmente amortiguada]
+ Estacionalidad para períodos múltiples enteros y no-enteros
